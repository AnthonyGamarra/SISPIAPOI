...
import { ExcelExportService } from '../formulacion-tabla/middlewares/excel-export-social.service';
import { ExcelImportService, ImportResult } from '../formulacion-tabla/middlewares/excel-import-social.service';
...
@Component({
  selector: 'app-formulacion-sociales-auto-tabla',
  templateUrl: './formulacion-sociales-auto-tabla.component.html',
  styleUrls: ['./formulacion-sociales-auto-tabla.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class FormulacionSocialesAutoTablaComponent implements OnDestroy {
  ...
  getFamilyNamesForDependency(dependencyName: string): string[] {
    return Object.keys(this.groupedActivitiesByDependency?.[dependencyName] || {});
  }

  getActivitiesForDependencyAndFamily(dependencyName: string, familyName: string): OperationalActivity[] {
    return this.groupedActivitiesByDependency?.[dependencyName]?.[familyName] || [];
  }
  ...
  private groupConsolidatedActivitiesByFamily(): void {
    ...
    this.consolidatedActivities.forEach(activity => {
      const familyName = activity.activityFamily?.name || 'Sin familia';
      
      if (!this.consolidatedActivitiesByFamily[familyName]) {
        this.consolidatedActivitiesByFamily[familyName] = [];
        this.consolidatedFamilyNames.push(familyName);
      }
      
      this.consolidatedActivitiesByFamily[familyName].push(activity);
    });
    
    // Ordenar familias alfabéticamente
    this.consolidatedFamilyNames.sort();
  }
