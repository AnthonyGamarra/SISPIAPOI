import { Component, Input, Output, EventEmitter, inject, OnDestroy, OnChanges, SimpleChanges, ViewChild } from '@angular/core';
...
export class FormulacionSocialesTablaComponent implements OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['currentFormulation']) {
      console.log('FormulacionSocialesTablaComponent: currentFormulation changed:', changes['currentFormulation'].currentValue);
      if (this.currentFormulation) {
        console.log('Cargando datos de prestaciones sociales para formulación:', this.currentFormulation.idFormulation);
        this.loadPrestacionesEconomicasData();
        this.loadOrderedActivityDetailNames();
      } else {
        console.log('Limpiando datos - currentFormulation es null');
        // Limpiar datos si no hay formulación
  ...
  private loadPrestacionesEconomicasData(): void {
    console.log('loadPrestacionesEconomicasData llamado con currentFormulation:', this.currentFormulation);
    
    if (!this.currentFormulation?.year || !this.currentFormulation?.modification) {
      console.log('No hay currentFormulation válida, saltando carga de datos');
      return;
    }
