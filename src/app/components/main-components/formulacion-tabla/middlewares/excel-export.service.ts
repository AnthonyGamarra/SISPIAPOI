...
export class ExcelExportService {
  ...
  private addSingleSheetWithAllActivities(
    ...
    this.setupSheetStylesAndProtection(worksheet, formulation);
  }
   ...
   * @param formulation Formulación actual para validaciones de trimestre
   */
  private setupSheetStylesAndProtection(worksheet: ExcelJS.Worksheet, formulation?: any): void {
    const headerFill: ExcelJS.FillPattern = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'B4C6E7' } };
    ...
    worksheet.eachRow((row, rowNumber) => {
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        ...
        // Estilos para el encabezado (fila 1)
        if (rowNumber === 1) {
        ...
        if (rowNumber > 1 && (colNumber === 16 || colNumber === 29)) {
          cell.fill = grayFill;
          cell.font = boldFont;
        }
      });
  ...
  private addProtectedConsolidatedSheet(workbook: ExcelJS.Workbook, consolidatedActivities: any[], formulation?: any): void {
    ...
    this.setupConsolidatedSheetStylesAndProtection(worksheet, formulation);
  }
   ...
   * @param formulation Formulación actual para validaciones de trimestre
   */
  private setupConsolidatedSheetStylesAndProtection(worksheet: ExcelJS.Worksheet, formulation?: any): void {
    const headerFill: ExcelJS.FillPattern = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'B4C6E7' } };
    const grayFill: ExcelJS.FillPattern = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'E6E6E6' } };
    const boldFont: Partial<ExcelJS.Font> = { bold: true };
    const blackBorder: Partial<ExcelJS.Borders> = {
    ...
    worksheet.eachRow((row, rowNumber) => {
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        ...
        if (rowNumber === 1) { // Fila de encabezado
          cell.fill = headerFill;
          cell.font = boldFont;
        } else if (colNumber === 16 || colNumber === 29) { // Columnas de totales
          cell.fill = grayFill;
          cell.font = boldFont;
        }
      });
