...
export class FormulacionSocialesTablaComponent implements OnDestroy {
  ...
  getQuarterlyGoal(activity: OperationalActivity, quarter: number): number {
    ...
    // For cumulative measurement types we should return the value of the last month of the quarter,
    // not the sum. Determine the measurement type from activity family parent if available.
    ...
    // If measurementTypeId === 2 or 3 => take the last month of the quarter (endMonth)
    if (measurementTypeId === 2 || measurementTypeId === 3) {
      const lastGoal = activity.monthlyGoals.find(g => g.goalOrder === endMonth);
      return lastGoal?.value || 0;
    }

    // Default behaviour: sum months inside the quarter
    let total = 0;
  ...
  getQuarterlyBudget(activity: OperationalActivity, quarter: number): number {
    ...
    // For budgets, follow the same rule: if the measurement type is cumulative (2 or 3)
    // use the last month's value of the quarter; otherwise sum the months.
    const fam = activity.activityFamily;
    let parentFam = fam;
    if (fam && fam.parentActivityFamily) {
      parentFam = fam.parentActivityFamily as any;
    ...
    if (measurementTypeId === 2 || measurementTypeId === 3) {
      const lastBudget = activity.monthlyBudgets.find(b => b.budgetOrder === endMonth);
      return lastBudget?.value || 0;
    }
  ...
  getConsolidatedQuarterlyGoals(consolidatedItem: any, quarter: number): number {
    ...
    // Determine measurement type from activityFamily (use parent if subfamily)
    ...
    // For cumulative measurement types, take the last month value inside the quarter
    if (measurementTypeId === 2 || measurementTypeId === 3) {
  ...
  private groupConsolidatedActivitiesByFamily(): void {
    // Nuevo: Generar familias/subfamilias y actividades ordenadas según ActivityDetail (id asc)
    this.consolidatedActivitiesByFamily = {};
    this.consolidatedFamilyNames = [];
    if (!this.consolidatedActivities?.length || !this.orderedActivityDetails?.length) return;

    // Crear un mapa de idActivityFamily a ActivityDetail
    const detailByFamilyId: { [id: string]: ActivityDetail } = {};
    this.orderedActivityDetails.forEach(detail => {
      if (detail.activityFamily && detail.activityFamily.idActivityFamily !== undefined) {
        detailByFamilyId[String(detail.activityFamily.idActivityFamily)] = detail;
      }
    });

    // Construir la jerarquía y numeración recorriendo los ActivityDetail ordenados
    let mainFamilyCounter = 1;
    const processedFamilyIds = new Set<string>();
    for (const detail of this.orderedActivityDetails) {
      const fam = detail.activityFamily;
      if (!fam) continue;
      const famId = String(fam.idActivityFamily);
      // Si ya procesamos esta familia, saltar
      if (processedFamilyIds.has(famId)) continue;
      // Si es familia principal (sin parent)
      if (!fam.parentActivityFamily) {
        const famKey = `${mainFamilyCounter}. ${fam.name}|${famId}`;
        // Actividades consolidadas para esta familia, ordenadas por idActivityDetail asc
        const items = this.consolidatedActivities
          .filter(item => String(item.activityFamily?.idActivityFamily) === famId)
          .sort((a, b) => {
            const famIdNum = Number(famId);
            const da = this.orderedActivityDetails.findIndex(d => d.name === a.name && d.activityFamily?.idActivityFamily === famIdNum);
            const db = this.orderedActivityDetails.findIndex(d => d.name === b.name && d.activityFamily?.idActivityFamily === famIdNum);
            return da - db;
          });
        if (items.length) {
          this.consolidatedActivitiesByFamily[famKey] = items;
          this.consolidatedFamilyNames.push(famKey);
        }
        processedFamilyIds.add(famId);
        // Subfamilias
        let subFamilyCounter = 1;
        for (const subDetail of this.orderedActivityDetails) {
          const subFam = subDetail.activityFamily;
          if (!subFam || !subFam.parentActivityFamily) continue;
          const subFamId = String(subFam.idActivityFamily);
          // Si ya procesamos esta subfamilia, saltar
          if (processedFamilyIds.has(subFamId)) continue;
          if (String(subFam.parentActivityFamily.idActivityFamily) === famId) {
            const subFamKey = `${mainFamilyCounter}.${subFamilyCounter}. ${subFam.name}|${subFamId}`;
            const subFamIdNum = Number(subFamId);
            const subItems = this.consolidatedActivities
              .filter(item => String(item.activityFamily?.idActivityFamily) === subFamId)
              .sort((a, b) => {
                const da = this.orderedActivityDetails.findIndex(d => d.name === a.name && d.activityFamily?.idActivityFamily === subFamIdNum);
                const db = this.orderedActivityDetails.findIndex(d => d.name === b.name && d.activityFamily?.idActivityFamily === subFamIdNum);
                return da - db;
              });
            if (subItems.length) {
              this.consolidatedActivitiesByFamily[subFamKey] = subItems;
              this.consolidatedFamilyNames.push(subFamKey);
            }
            processedFamilyIds.add(subFamId);
            subFamilyCounter++;
          }
        }
        mainFamilyCounter++;
    ...
    // Si hay familias sin padre ni nombre, agregarlas al final
    const usedIds = this.consolidatedFamilyNames.map(key => key.split('|')[1]);
    for (const detail of this.orderedActivityDetails) {
      const fam = detail.activityFamily;
      if (!fam) continue;
      const famId = String(fam.idActivityFamily);
      if (!usedIds.includes(famId)) {
        const orphanKey = `X. ${fam.name}|${famId}`;
        const famIdNum = Number(famId);
        const orphanItems = this.consolidatedActivities
          .filter(item => String(item.activityFamily?.idActivityFamily) === famId)
          .sort((a, b) => {
            const da = this.orderedActivityDetails.findIndex(d => d.name === a.name && d.activityFamily?.idActivityFamily === famIdNum);
            const db = this.orderedActivityDetails.findIndex(d => d.name === b.name && d.activityFamily?.idActivityFamily === famIdNum);
        ...
        if (orphanItems.length) {
          this.consolidatedActivitiesByFamily[orphanKey] = orphanItems;
          this.consolidatedFamilyNames.push(orphanKey);
        }
      }
    }
  }
