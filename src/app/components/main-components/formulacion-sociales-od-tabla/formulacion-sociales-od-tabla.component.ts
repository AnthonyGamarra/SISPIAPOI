...
@Component({
  selector: 'app-formulacion-sociales-od-tabla',
  templateUrl: './formulacion-sociales-od-tabla.component.html',
  styleUrls: ['./formulacion-sociales-od-tabla.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class FormulacionSocialesOdTablaComponent implements OnInit, OnChanges {
  ...
  private activityFamilyService = inject(ActivityFamilyService);
  ...
  loadOperationalActivities(): void {
    ...
    this.operationalActivityService.searchByFormulation(this.idFormulation).subscribe({
      next: (data) => {
        // Ordenar primero por unidad operativa, luego por nombre de prestación social
        const sortedData = this.sortActivitiesByFamilyAndName(data);

        this.products = sortedData.map(activity => {
          // Initialize goals for quarterly view (legacy)
          ...
          // Initialize monthly goals for new monthly view
          const loadedMonthlyGoals = activity.monthlyGoals || [];
          activity.monthlyGoals = Array.from({ length: 12 }, (_, i) => {
            const existingGoal = loadedMonthlyGoals.find(g => g.goalOrder === i + 1);
            return existingGoal
              ? { ...existingGoal, operationalActivity: { idOperationalActivity: activity.idOperationalActivity } as OperationalActivity }
              : { goalOrder: i + 1, value: 0, operationalActivity: { idOperationalActivity: activity.idOperationalActivity } } as MonthlyGoal;
          });
          activity.monthlyGoals.sort((a, b) => a.goalOrder - b.goalOrder);

          // Initialize monthly budgets for new monthly view
          const loadedMonthlyBudgets = activity.monthlyBudgets || [];
          activity.monthlyBudgets = Array.from({ length: 12 }, (_, i) => {
            const existingBudget = loadedMonthlyBudgets.find(b => b.budgetOrder === i + 1);
            return existingBudget
              ? { ...existingBudget, operationalActivity: { idOperationalActivity: activity.idOperationalActivity } as OperationalActivity }
              : { budgetOrder: i + 1, value: 0, operationalActivity: { idOperationalActivity: activity.idOperationalActivity } } as MonthlyBudget;
          });
          activity.monthlyBudgets.sort((a, b) => a.budgetOrder - b.budgetOrder);
          activity.goals.sort((a, b) => a.goalOrder - b.goalOrder);
  ...
  private notifyActivityChangesToModal(): void {
    // Ya no hay modales para notificar
  }
  // --- Action Methods ---
  agregarActividad(): void {
    ...
    // Aplicar ordenamiento después de agregar la nueva actividad
    this.products = this.sortActivitiesByFamilyAndName(this.products);
  ...
  private validateActivity(product: OperationalActivity): boolean {
    // Validación básica sin middleware
    return !!(product.name && product.measurementUnit);
  }
  ...
  private _generateSapCodeAndCorrelative(activity: OperationalActivity): Observable<{ sapCode: string, correlativeCode: string }> {
    // Generación básica sin middleware
    return of({
      sapCode: 'SAP',
      correlativeCode: 'COR'
    });
  ...
  onActivitiesCreatedFromConsolidated(activities: OperationalActivity[]): void {
    ...
    if (newActivities.length > 0) {
      ...
      // Aplicar ordenamiento después de agregar nuevas actividades
      this.products = this.sortActivitiesByFamilyAndName(this.products);
    }
      ...
      this.operationalActivityService.create(activityToCreate).subscribe({
        next: (createdActivity) => {
          ...
          if (tempIndex >= 0) {
            ...
            // Aplicar ordenamiento después de actualizar la actividad
            this.products = this.sortActivitiesByFamilyAndName(this.products);
          } else {
            // Si no se encuentra, agregar la actividad creada
            this.products.push(createdActivity);
            // Aplicar ordenamiento después de agregar la actividad
            this.products = this.sortActivitiesByFamilyAndName(this.products);
          }
        },
        error: (err) => {
  ...
  // Métodos para calcular totales de metas y presupuestos mensuales
  getTotalMonthlyGoals(product: OperationalActivity | null): number {
    if (!product?.monthlyGoals) return 0;
    return product.monthlyGoals.reduce((total, goal) => total + (goal?.value || 0), 0);
  }

  getTotalMonthlyBudgets(product: OperationalActivity | null): number {
    if (!product?.monthlyBudgets) return 0;
    return product.monthlyBudgets.reduce((total, budget) => total + (budget?.value || 0), 0);
  }

  // Método para obtener el nombre de la unidad operativa (family)
  getActivityFamilyName(product: OperationalActivity | null): string {
    return product?.activityFamily?.name || 'Sin unidad operativa';
  }

  // Método para ordenar las actividades por unidad operativa y luego por nombre
  private sortActivitiesByFamilyAndName(activities: OperationalActivity[]): OperationalActivity[] {
    return activities.sort((a, b) => {
      // Primer criterio: unidad operativa
      const familyA = a.activityFamily?.name || 'zzz'; // Poner al final las sin unidad operativa
      const familyB = b.activityFamily?.name || 'zzz';
      const familyComparison = familyA.localeCompare(familyB);
      
      if (familyComparison !== 0) {
        return familyComparison;
      }
      
      // Segundo criterio: nombre de la prestación social
      const nameA = a.name || '';
      const nameB = b.name || '';
  ...
  isMonthEditable(monthIndex: number): boolean {
    if (!this.currentFormulation?.quarter) {
      return true; // Si no hay quarter definido, todos los meses son editables
    }

    const quarter = this.currentFormulation.quarter;
    const startMonth = (quarter - 1) * 3; // Mes inicial del trimestre actual

    // Solo los meses del trimestre actual y posteriores son editables
    // quarter = 1: meses 0-11 editables (enero-diciembre)
    // quarter = 2: meses 3-11 editables (abril-diciembre), bloquea 0-2 (enero-marzo)
    // quarter = 3: meses 6-11 editables (julio-diciembre), bloquea 0-5 (enero-junio)
    // quarter = 4: meses 9-11 editables (octubre-diciembre), bloquea 0-8 (enero-septiembre)
